...
def denormalize(tensor):
    tensor = tensor * Tensor([0.2470, 0.2435, 0.2616])[:, None, None]
    tensor = tensor + Tensor([0.4914, 0.4822, 0.4465])[:, None, None]
    return tensor
...
def sort_training_data(teacher_model, train_dataloader, val_dataloader, params):
    ...
    for i in range(10):
        train_images_01.append(imgs[i][0:1000])
        train_images_02.append(imgs[i][1000:2000])
        train_images_03.append(imgs[i][2000:3000])
        train_images_04.append(imgs[i][3000:4000])
        train_images_05.append(imgs[i][4000:5000])
    ...
    for i in range(10):
        train_labels_01 += [i for j in range(1000)]
    ...
    for idx, (img, target) in enumerate(train_loader, start=1):
        ...
        if idx % args.print_freq == 0:
            ...
            log_list.append('Epoch[{0}]:[{1:03}/{2:03}],'
                ...
                epoch, idx, len(train_loader), batch_time=batch_time, data_time=data_time,
                cls_losses=cls_losses, st_losses=st_losses, top1=top1, top5=top5))
    with open('./log_train_new.txt', 'a+') as f:
...
def traintemp(train_loader, nets, tempnets, optimizer, criterions, epoch, batchsize):
    ...
    for idx, (img, target) in enumerate(train_loader, start=1):
        ...
        temp_loss = criterionLogits(output_s, output_t.detach())
